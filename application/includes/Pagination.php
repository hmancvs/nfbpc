...
/**
	...
	function getAlphabetString(){
		...
		$alphabet = range('a', 'z');
		$alphabet_string = '<ul class="alphabets"><li class="all" title="List All"><a class="gonowhere" >All</a></li>';
		
		// debugMessage($currenturl);
		foreach ($alphabet as $letter) {
			// debugMessage($_SERVER['REQUEST_URI'].'/alphabet/'.$letter);
			// $alphabet_link_array[] = '<a href="javascript: void(0);" title="Browse by Letter '.$letter.'" id="'.$letter.'" class="alphabet">'.$letter.'</a>';
			$alphabet_string .= '<li class="gonowhere" title="Browse by Letter '.$letter.'" ><a href="" class="gonowhere" id="'.$letter.'">'.strtoupper($letter).'</a></li>';
		} 
		$alphabet_string .= '</ul>';
