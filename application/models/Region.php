...
class Region extends BaseEntity {
	...
	function afterSave(){
		...
		$view = new Zend_View();
		$url = $view->serverUrl($view->baseUrl('location/view/id/'.encode($this->getID())));
		$usecase = '4.1';
		$module = '4';
		$type = LOCATION_CREATE;
		$details = "Region <a href='".$url."' class='blockanchor'>".$this->getName()."</a> created";
	...
	function beforeUpdate(){
		...
		$region = new Region();
		$region->populate($this->getID());
		$this->setPreUpdateData($region->toArray()); // debugMessage($this->toAray());
		// exit;
		return true;
	}
	...
	function afterUpdate(){
		...
		$view = new Zend_View();
		$url = $view->serverUrl($view->baseUrl('location/view/id/'.encode($this->getID()).'/type/0'));
		$usecase = '4.2';
		$module = '4';
		$type = LOCATION_UPDATE;
		$details = "Region <a href=".$url." class='blockanchor'>".$this->getName()."</a> updated.";
